...
public class OS_EntityDiamondSpider extends EntitySpider
{
	
	public OS_EntityDiamondSpider(World world) 
	{
		super(world);
	}
		...
		if(! (this.isBurning()))
		{
			if(amountToDrop != 0)
			{
				if(lootEnchantLevel == 0)
				{
					if(ranNum == 1)
					{
						this.dropItem(MCUB_Item.diamondNugget.itemID, amountToDrop);
					}
					this.dropItem(Item.spiderEye.itemID, 1);
				}
				else
				{
					this.dropItem(MCUB_Item.diamondNugget.itemID, amountToDropWithLootEnchant);
